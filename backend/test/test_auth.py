...
def test_forgot_password_send_link(client):
    ...
    assert res.status_code == 200
    assert 'reset link' in res.get_json()['message'] or "will be sent" in res.get_json()['message']
...
def test_reset_password_invalid_token(client):
    ...
    # 有些時候訊息只會回 "Reset link is invalid or expired"
...
def test_forgot_password_invalid_email_format(client):
    ...
    assert res.status_code == 200  # 系統安全設計還是會回 200
...
def test_reset_password_missing_fields(client):
    ...
    assert res.status_code == 400
    # 缺 token
    res = client.post('/auth/reset_password', json={'password': 'xxx'})
    assert res.status_code == 400
...
def test_reset_password_expired_token(client):
    ...
    db.session.add(token)
    db.session.commit()
    res = client.post('/auth/reset_password', json={'token': 'expiredtoken', 'password': 'newpw'})
    assert res.status_code == 400
    assert 'invalid or expired' in res.get_json()['error']
